...
pub fn lower_function(func: &ItemFn, backend: &str) -> syn::Result<TokenStream> {
    ...
    let ir_path = resolve_crate("axiom-ir", "axiom_ir");
    let zk_path = resolve_crate("axiom-zk", "axiom_zk");
    ...
    gen.declare_params(func)?;
    gen.lower_block(&func.block)?;
    if has_return && !gen.output_bound {
        return Err(Error::new(
            func.sig.ident.span(),
            "this function must end with a `return <expr>;` or a trailing expression to become a public output",
        ));
    }
    let build_body = gen.tokens;
