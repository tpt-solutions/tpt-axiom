...
- **Rust application logic** is ordinary Rust using `Fuzzy<T>` / `Distribution<T>`
  ...
  (ZK path).
- **The mathematical IR generator** (`axiom-ir`) lowers both paths into a
...
- **The optimized output** is either ordinary compiled Rust (for the
  ...
  verifying keys, produced by a backend adapter (`axiom-backend-halo2`,
  `axiom-backend-arkworks`, `axiom-backend-sp1`) implementing the `ZkBackend`
  trait from `axiom-zk`.
...
```text
                     ┌────────────┐
                     │ axiom-core │  (Fuzzy<T>, Distribution<T>)
                     └─────┬──────┘
                           │
                     ┌─────▼──────┐
...
┌─────────────┐     ┌──────────┐     ┌─────────────────────┐
│axiom-macros │────>│ axiom-ir │<────│ axiom-zk (ZkBackend) │
└─────────────┘     └────┬─────┘     └──────────┬───────────┘
                          │                      │
                          │           ┌──────────┼───────────────┐
                          │           ▼           ▼               ▼
                          │  axiom-backend-halo2  axiom-backend-arkworks  axiom-backend-sp1
                          │
                          └──> consumed by axiom-cli (build-time driver)
```

- `axiom-core` has no dependency on the ZK stack — probabilistic types remain
  usable (and publishable) independently of zero-knowledge concerns.
- `axiom-ir` is the single shared IR consumed by both the macro layer
  (`axiom-macros`) and the backend abstraction layer (`axiom-zk` and its
...
1. **Phase 1** — `axiom-core` (top box): `Fuzzy<T>`/`Distribution<T>` and
   error-propagation arithmetic. No IR, no ZK.
2. **Phase 2** — `axiom-ir` + `axiom-macros`: lowering Rust to the shared IR,
   with `axiom-zk`'s `ZkBackend` trait defined as the consumption contract.
...
4. **Phase 4** — `axiom-backend-*`: concrete `ZkBackend` implementations
